...
1. **单条 TCP 连接受拥塞控制（BDP 瓶颈）限制**，跨网延迟下通常只有 2-5 MB/s —— 8 路并发把有效速率变成多条连接之和
...
直链指向的 CDN 节点是随机分配的（"连接级抽奖"）。播放结束后若速度持续低于阈值，后台静默测速多个候选节点、淘汰慢节点——仅在文件空闲 30 秒后进行，绝不与正在播放的请求抢带宽。

### 3. 64KB 首段快速出画 + 滑动窗口调度

请求首段只下 64KB：TLS 完成后初始拥塞窗口内一趟传完，TTFB 最小，播放器尽快解析 moov 出画面。首段独享全部带宽，之后才放开 8 路并发。
